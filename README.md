...
1. 실행 전략: 불확실성 속에서 현명하게 판단하는 질문들 (출처: [언제 멈추고 언제 밀어붙여야 할까(번역글)](https://news.hada.io/topic?id=21613))
  1. 아래와 같은 질문과 전략을 통해, 멈출지 계속할지 조금 더 객관적으로 판단할 수 있다.
  1. 타임박스(Timebox): 일정 기간 내에 개선이 없으면 미련 없이 그만두겠다고 데드라인을 정한다.
  1. 재미와 배움: 여전히 이 프로젝트가 재미있고, 배우는 게 있는가?
  1. 진행 속도: 최근 진행이 빨라지고 있는가, 느려지고 있는가?
  1. 제3자 시각: 외부인의 피드백으로 자신을 점검한다.
  1. 매몰비용 무시: 이미 쓴 시간·돈에 휘둘려 결정하지 않는다.
  1. 팀의 에너지: 팀이 더 이상 확신이나 아이디어가 없다면, 방향 전환(pivot)이나 종료를 고민한다.
  1. 12개월 뒤 상상: 1년 뒤에도 이 일을 하고 있을 자신이 있는가?
  1. 제품 경쟁력: 우리 제품이 시장 대안보다 확실히 낫거나 곧 나아질 것 같은가?
  1. 입증된 사례(Existence proof): 남이 비슷한 성공을 이미 해냈는가?
  1. 가치·개선·의지: (a) 충분히 가치 있는가, (b) 개선 가능성이 있는가, (c) 내가 여전히 에너지가 있는가?
  1. 기회비용(Opportunity Cost): 이 일에 집착하느라 더 중요한 기회를 놓치고 있지 않은가?
  1. 결과와 보상: 실패 시 불이익, 성공 시 보상은 무엇인가?
  1. 두려움 극복: 두려움 때문에 결정을 미루지 않는다.
  1. 내면의 소리: 혼자 산책하며 내면의 진짜 답을 들어본다.
  1. 처음의 ‘이유(Why)’: 시작할 때의 동기가 지금도 유효한가?
