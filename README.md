...
1. 학습에는 의도가 필요하다
1. 여기서 진정한 도전 과제 중 하나는 인공지능이 주의력이라는 인간의 가치와 그로부터 비롯되는 개성을 훼손하는 방식이다. - nytimes
1. 혼자서 오래 글을 쓰던 사람들은 인정의 계기가 없으면 자기에 대해서 계속 의심을 하게 되잖아요. - 현호정x하미나 라이브 북토크
1. 뇌는 적절한 상황에 적절한 감정을 느껴야 한다 - 서은국
1. 당신은 우리가 누구이며 우리가 왜 여기 있는지에 대한 가장 근본적 의문을 다시 생각하게끔 만들었습니다. 그 점에서 우리는 고마운 마음입니다. - 카일 포프 컬럼비아 저널리즘 리뷰 발행인
1. 어떤 의자에 앉아 판결하고 있는지 명심하라 - 헤라르트 다비트 (캄비세스 왕의 심판)
1. 잔인하기 짝이 없는 이들은 솔직함에서 자신을 따라올 자가 없다는 사실에 자부심을 갖는다. - 테네시 윌리엄스
  - 당신이 믿을만한 사람인지 아닌지는 당신이 진실만을 말하는지 여부가 아니다. 그보다는 말하기로 선택한 것들의 동기가 바람직한 지가 더 중요하다. 본연의 사실에만 충실하기 보다 최소한 상대방, 상대방의 욕구, 그리고 자신의 염원과 이미지를 충분히 고려하라.
...
1. 돈과 사랑의 벤다이어그램. 선량하면서 유능한 것. 공존하기 어려워 보이는 이 두 속성을 모두 가지는 대표적인 사례는 영화에 등장하는 영웅 캐릭터다. 현실에서도 이 두 속성을 모두 가지는 게 불가능한 것은 아니지만 연구에 따르면 사람들은 여성을 좀 더 냉혹하게 평가하는 경향이 있다.
1. 평판이란 당신을 평가하는 관객들이 부여하는 것이고, 이들 관객에게는 각각의 편견과 오류가 존재한다.
1. 최근 정치판이 격동에 휘말리면서 자신의 입장을 뒷받침하기 위해 진실보다는 적당한 증거를 찾는 '대안적  진실'의 세상이 도래했다.
1. 각자의 평가가 맞다는 확신이 들 때마다 인간은 자신의 입맛대로 내려진 평가에 확실한 근거를 생성하려는 내적 체계가 갖춰져 있다는 사실을 떠올려야 한다.
1. 인간은 (무의식 중에도) 끊임없이 평가한다는 사실을 인지하고, 나아가 그 평가라는 게 기껏해야 편협할 뿐이어서 해당 대상이나 상황에 대해 알아갈수록 끊임없이 평가를 수정하는 게 마땅하다는 사실을 기억하자.
1. 아고다는 피쳐 스토어를 Feast에서 Rust로 전환하여 높은 성능과 안정성을 달성했다. Rust의 러닝 커브는 이미 Rust를 도입해 사용하는 타팀과 Copilot, 그리고 친절한 Rust 컴파일러의 도움으로 해결할 수 있었다.
1. 벡터 클럭이 의도한 효과를 내고 업데이트 손실을 방지하려면, vclock의 필드로 표현되는 요소가 실제 동시성 단위여야 한다.
1. 벡터 클럭은 어렵다. 완벽하게 구현하더라도 개방형 시스템에서 무한한 정보 증가 없이 인과 관계에 대한 완벽한 정보를 가질 수는 없다.
1. 그러나 증명이 불가능해도 그런 관념(종교 원형)을 계발해 나가야 하는 중요한 경험적인 이유가 있다. 그러한 관념이 유용한 것으로 판명되었다는 것이 바로 그 이유이다. 인간은 자기 삶에 의미를 부여하고 우주 내에서 자기의 위치를 찾을 수 있도록 해줄 일반적인 관념과 신념을 필요로 하며 그것을 통해 엄청난 힘을 얻는다. 천산만고를 겪는다 해도, 그것이 납득 가능하다고 생각하는 한 인간은 그런 고뇌까지도 견다어 낸다. - 인간과 상징 (칼 융)
1. 융은 틈만 나면 인간이 자연을 지배하고 있는 것처럼 보이지만 사실은 아직 자기의 본성도 지배하지 못했다는 점을 지적하고는 했다.
1. 메달리온 아키텍쳐는 데이터 모델링이 아니다. 메달리온 아키텍쳐는 데이터 의 라이프 사이클을 의미하고 데이터 모델은 데이터의 구조와 관계를 정의한다. 데이터 모델은 비즈니스에서 고객이 무엇이고 어떻게 식별하는지, 데이터셋의 그레인은 무엇인지, 어떤 속성이 느리게 변하며 이력을 어떻게 추적할지, 제약 조건을 어디에 두고 무결성을 강제할지 등을 정의한다. 메달리온은 브론즈에서 골드로 이동하며 최종 사용자에게 필요한 데이터를 쉽게 사용하고 가치있게 사용할 수 있는 방식으로 제공된다. - 조 라이스
  * 브론즈 : 최소한의 변화만 수행하고 스키마 드리프트를 지원
  * 실버 : 중복 제거, 타입 지정, 아이디 표준화 및 키 설정, 데이터 품질 규칙 등을 적용
  * 골드 : 대시보드, 리포트 등을 위해 특별히.모델링된 데이터가 위치
  * 데이터 모델링 전술 : 스타 스티마 (BI 슬라이스 앤 다이스), OBT (One Big Table), 데이터 볼트, 데이터 마트
1. Daft, DuckDB는 2코어, 16GB로 150GB CSV 파일을 변환할 수 있다. Polars, CHTP는 32코어 이상에서 정상적으로 동작했다. 하지만 이는 근본적인 문제가 아니며 몇 주에서 몇 개월 후에는 결과가 달라질 수 있다.
...
* 확장성 한계
  * 모든 메타데이터를 메모리에 유지하는 방식은 매우 좋은 성능을 제공했지만 대신 확장성을 저해하는 단점이 있다. 오브젝트 개수가 3억5천만개를 넘으면 클러스터를 재시작하는 시간이 길어지고 데이터노드의 블록리포트를 처리하는 데도 긴 시간이 소요된다. 16TB의 힙을 지원하는 ZGC를 사용하도 재시작 문제는 여전히 해결되지 않으며 최대 7억개의 오브젝트가 한계다. 이 문제에 대한 명확한 해결책은 존재하지 않는다.
  * 확장성 문제는 결국 오존이라는 차세대 스토리지 개발로 이어진다. 오존은 네임노드와 다르게 네임스페이스와 블록 스페이스를 분리해서 이 문제를 해결한다.
...
* 핵심 엔티티
  * 볼륨 : S3의 사용자 계정
  * 버킷 : S3의 버킷. 객체 저장소/파일 시스템 모델 중 선택할 수 있다.
  * 키 : 실제 데이터가 저장되는 단위
* 블록 크기는 HDFS와 동일하다.
* 객체 저장소 모델이므로 키 간의 관계나 디렉토리/파일, 부모/자식 관계가 없는 플랫한 키-값 저장소 구조를 가진다. Hadoop Compatible File System API를 지원하므로 계층적 파일 시스템처럼 접근할 수 있다.
* 마스터 노드
  * Ozone Manager : 네임스페이스만 관리. 키 이름과 블록 ID 간 매핑만 유지한다.
  * Storage Container Manager : 블록 스토리지를 제공. 모든 데이터 노드, 블록 저장과 접근 방삭을 처리한다.
  * 합의 모델은 RAFT (Apache Ratis)를 사용한다.
  * OM과 SCM이 저장하는 메타데이터는 내장된 RocksDB를 이용하며 손쉽게 다른 DB로 교체할 수 있다. 성능을 위해 RocksDB는 SSD나 NVNe 사용을 권장한다.
* 컨테이너 모델 : 개별 블록을 추적하는 대신 블록들을 컨테이너로 그룹화한다. 블록을 읽으려면 컨테이너 위치를 확인한 후 데이터 노드에 가서 블록을 요청한다. 데이터 노드는 HDFS의 블록 리포트 대신 훨씬 가벼운 컨테이너 리포트를 전송한다. 기본 컨테이너 크기는 5GB이며 이 값에 도달하면 더 이상 쓰기가 허용되지 않는다.
* 파일과 컨테이너의 관계 : 하나의 파일에 속한 블록은 여러 컨테이너에 분산될 수 있다.
* 오존 선택 시 고려 사항 : 마스터 노드에 SSD 필요. 현재 HDFS와 거의 동등한 성능 수준에 도달. 메타데이터 접근에 디스크 I/O가 필요하다는 점 외에는 오존은 모든 HDFS 기능을 제공한다. 
* HDFS의 스몰 파일 문제 해결
  * 네임스페이스와 블록 스페이스 분리하고 객체 저장소 모델 도입
  * 부분적인 메타데이터만 메모리에 로드하고 RocksDB를 이용해서 모든 정보를 디스크에 저장한다. 변경은 RocksDB에 기록되며, 파일을 읽을 때는 RocksDB에서 메타데이터를 읽어오므로 마스터 노드에 메모리 압박이 없다. 자주 읽는 파일은 캐싱을 하지만 그렇지 않은 파일은 디스크I/O가 발생할 가능성이 있다. 
